  ...
  try {
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      if (result.deletedCount !== 1) {
        return res
          .status(404)
          .json({ error: "Unable to find note with given ID belonging to this user." });
      }
      res.json({ response: `Documnent with ID ${noteId} properly deleted` });
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
  ...
  try {
    ...
    const { title, content } = req.body;
      ...
      if (!title && !content) {
        return res
          .status(400)
          .json({ error: "At least one of title or content needs to be present in the body." });
      }
